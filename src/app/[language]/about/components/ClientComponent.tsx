...
export default function ClientComponent({ language }: { language: string }) {
	...
	useEffect(() => {
		if (data?.about_page?.page_main_settings?.background_color) {
			// Устанавливаем CSS переменную --backgroundColor в :root
			document.documentElement.style.setProperty("--backgroundColor", data.about_page.page_main_settings.background_color);
			document.documentElement.style.setProperty("--backgroundColorTransparent", data.about_page.page_main_settings.background_color + "40");
		} else {
		...
		if (data?.about_page?.page_main_settings?.text_is_light) {
			// Устанавливаем CSS переменную --backgroundColor в :root
	...
	}, [data?.about_page?.page_main_settings?.background_color, data?.about_page?.page_main_settings?.text_is_light]);
